					...
					else if (!strcmp(clicked_button->group, "building types"))
					{
						for (int i = 0; i < sizeof(building_types) / sizeof(Building); i++)
						{
							if (!strcmp(building_types[i].name, clicked_button->name))
							{
								...
								new_building.milkrun_vehicle_limit = building_types[i].milkrun_vehicle_limit;
...
void Simulation()
{
	// Get elapsed time since last render cycle
	long elapsed_time = Get_Elapsed_Time();
	mouse_left_clicked = false;

	// Simulation
	Cargo_Loading_Handler();
	Production_Handler(elapsed_time);
	Delivery_Handler(elapsed_time);
}
